...
class Comment extends Mccms_Controller {
    ...
    public function lists() {
        if(Pl_Mode == 1) get_json(array('html'=>'Comments on the site are closed','msg'=>'Comments are closed'),1);
    ...
    public function add() {
        ...
        if(($row['addtime']+Pl_Time) > time()) get_json('Please rest for a while before commenting');
        //判断每天评论数量
        ...
        if(($num1+$num2) >= Pl_Add_Num) get_json('You have reached the limit of comments today, come back tomorrow');
        //过滤评论内容
        ...
        if($res){
            $d['code'] = 1;
            $d['msg'] = $cid > 0 ? 'Reply to comment successfully' : 'The comment was successful';
            //增加回复次数
            if($cid > 0){
    ...
    public function zan() {
        ...
        if($id == 0) get_json('Parameter error');
        if(!$this->users->login(1)) get_json('login timeout');
        $uid = $this->cookie->get('user_id');
