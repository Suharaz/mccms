...
class Pay extends Mccms_Controller {
    ...
    public function index() {
        ...
        //判断登陆
        if(!$this->users->login(1)) get_json('login timeout!!!');
        ...
        //VIP
        $arr['pay']['vip'] = array(
            array('day'=>30,'rmb'=>Pay_Vip_Rmb1,'name'=>'Monthly VIP','txt'=>'Valid for 30 days'),
            array('day'=>90,'rmb'=>Pay_Vip_Rmb2,'name'=>'Quarterly VIP','txt'=>'Valid for 90 days'),
            array('day'=>180,'rmb'=>Pay_Vip_Rmb3,'name'=>'Six months VIP','txt'=>'Valid for 180 days'),
            array('day'=>365,'rmb'=>Pay_Vip_Rmb4,'name'=>'Annual VIP','txt'=>'Valid for 365 days'),
        );
        //月票
    ...
    public function save() {
        ...
        if($day == 0) $day = 30;
        if($type == 'cion' && ($rmb < 1 || $rmb >9999)) get_json('wrong amount!!!');
        if($type == 'cion' && $rmb < Pay_Rmb_Min) get_json('Minimum recharge amount'.Pay_Rmb_Min.'元');
        if($type == 'ticket' && $num < 1) get_json('The number of monthly tickets is wrong!!!');
		if($type == 'card' && empty($card)) get_json('Card password cannot be empty!!!');
        if($type == 'vip' && !in_array($day, $dayarr)) get_json('Vip time error!!!');
        ...
        //充值类型
		if($type == 'card'){
            if(!ctype_alnum($card)) get_json('Card password cannot be empty');
			$row = $this->mcdb->get_row_arr('card','*',array('pass'=>$card));
			if(!$row) get_json('Kami does not exist!!!');
			if($row['uid'] > 0) get_json('Cards cannot be reused!!!');
			if($row['sid'] == 1){ //VIP卡
				$name = 'Card secret recharge VIP member success';
                $text = 'You have successfully purchased using the passcode'.$row['day'].'天Vip会员';
                $edit['vip'] = 1;
				...
				$name = 'Card password recharge gold coins successfully';
                $text = 'You have successfully recharged using the card code'.$row['cion'].'coins';
            ...
            $d['code'] = 1;
            $d['pay'] = 0;
            $d['msg'] = 'Recharge successful, please wait...';
            get_json($d);
        }elseif($type == 'ticket'){
            $rmb = $num;
            $txt = 'Buy'.$num.'monthly ticket';
            $zd = array('zd'=>'ticket','num'=>$num);
        }elseif($type == 'vip'){
            $rarr = array('30'=>Pay_Vip_Rmb1,'90'=>Pay_Vip_Rmb2,'180'=>Pay_Vip_Rmb3,'365'=>Pay_Vip_Rmb4);
            $rmb = $rarr[$day];
            $txt = 'Buy '.$day.'Day VIP member';
            $zd = array('zd'=>'vip','day'=>$day);
        }else{
            $txt = 'Buy '.$rmb*Pay_Rmb_Cion.' '.Pay_Cion_Name;
        ...
        if($pay == 'cion'){
            ...
            if($cion > $user['cion']) get_json(Pay_Cion_Name.'Insufficient, please recharge first!!!');
            //购买月票
            if($type == 'ticket'){
                $edit['ticket'] = $user['ticket']+$num;
                $name = 'Successfully purchased a monthly pass';
                $text = '您花费'.$cion.Pay_Cion_Name.'，successfully purchased'.$num.'monthly ticket';
            }else{
                $name = '购买VIP会员成功';
                $text = '您花费'.$cion.Pay_Cion_Name.'，successfully purchased'.$day.'Day VIP member';
                $edit['vip'] = 1;
                if($user['viptime'] > time()){
            ...
            $d['code'] = 1;
            $d['pay'] = 0;
            $d['msg'] = 'Purchase successful, please wait......';
            get_json($d);
        }
        //记录订单
        $add2['dd'] = date('YmdHis').rand(1111,9999);
        ...
        if(defined('MOBILE')){
            ...
            $d['msg'] = 'Payment address obtained successfully';
        }else{
            //二维码地址
            if($pay == 'alipay'){
            ...
            $d['msg'] = 'QR code obtained successfully';
        }
        get_json($d);
    }
