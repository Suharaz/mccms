...
class Rend extends Mccms_Controller {
    ...
    public function gift(){
        ...
        if($mid == 0 && $bid == 0) get_json('Parameter error');
        if($mid > 0){
    ...
    public function ticket_send(){
        ...
        if(($mid == 0 && $bid == 0) || $ticket == 0) get_json('Parameter error');
        $log = $this->users->login(1);
        ...
        if($rowm['uid'] > 0 && $fccion > 0 && $rowm['uid'] != $uid){
            $add2['uid'] = $rowm['uid'];
            $add2['text'] = 'receive '.$ticket.'Reward with a Power';
            $add2['mid'] = $mid;
            $add2['bid'] = $bid;
            $add2['cion'] = $fccion;
            $add2['zcion'] = $ticket;
        ...
        get_json(array('msg'=>'Thank you for your precious Power, the author will continue to work hard','uticket'=>$xticket,'ticket'=>($rowm['ticket']+$ticket)),1);
    }
    ...
    public function gift_send(){
        ...
        $bid = (int)$this->input->get_post('bid');
        if($num == 0) $num = 1;
        if($gid == 0 || ($mid == 0 && $bid == 0)) get_json('Parameter error');
        $log = $this->users->login(1);
        if(!$log) get_json('登陆超时');
        $uid = $this->cookie->get('user_id');
        if($mid > 0){
        ...
        get_json(array('msg'=>'Thanks for your support~~','mcion'=>($rowm['cion']+$cion),'cion'=>$xcion),1);
    }
    //评分
    public function score_send(){
        ...
        if($fen == 0 || $did == 0) get_json('Parameter error');
        if($type == 'book'){
            $table = 'book_score';
            $zd = 'bid';
        }else{
    ...
    public function read() {
        ...
        if($type != 'book') $type = 'comic';
        if($did == 0 || $cid == 0) get_json('Parameter error!!!');
        $uid = $this->cookie->get('user_id');
