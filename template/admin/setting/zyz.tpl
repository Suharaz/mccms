...
<div class="breadcrumb-nav">
    <span class="layui-breadcrumb">
        <a>系统配置</a>
        <a><cite>Resource station configuration</cite></a>
    </span>
    <a class="layui-btn layui-btn-sm" style="line-height:1.6em;margin-top:3px;float:right" onclick="location.reload()" title="to refresh"><i class="layui-icon layui-icon-refresh" style="line-height:30px"></i></a>
</div>
            ...
            <form class="layui-form" action="<?=links('setting','zyz_save')?>">
                <div class="layui-tab layui-tab-brief" lay-filter="setting">
                    <ul class="layui-tab-title">
                        <li class="layui-this">Comics Resources</li>
                        <li>Novel Resources</li>
                    </ul>
                            ...
                            <div class="layui-text laytps" style="max-width: 700px;padding-top: 25px;">
                                <div class="layui-form-item w120">
                                    ...
                                    <div class="layui-input-inline" style="display: block;width: auto;float: none;">
                                        <input type="radio" name="comic[init]" value="0" title="Close"<?php if($zyz['comic']['init'] == 0) echo ' checked';?>>
                                        <input type="radio" name="comic[init]" value="1" title="On"<?php if($zyz['comic']['init'] == 1) echo ' checked';?>>
                                    </div>
                                </div>
                                <div class="layui-form-item w120">
                                    <label class="layui-form-label layui-form-required">list per page:</label>
                                    <div class="layui-input-block">
                                        <input type="text" name="comic[size]" placeholder="The list shows the number of comics per page" value="<?=$zyz['comic']['size']?>" class="layui-input" lay-verify="required" required/>
                                    </div>
                                </div>
                                <div class="layui-form-item w120">
                                    <label class="layui-form-label layui-form-required">image domain name:</label>
                                    <div class="layui-input-block">
                                        <input type="text" name="comic[picurl]" placeholder="Image domain name, if left blank, configure according to the attachment" value="<?=$zyz['comic']['picurl']?>" class="layui-input"/>
                                    </div>
                                </div>
                                <div class="layui-form-item w120">
                                    <label class="layui-form-label layui-form-required">Category ID filtering:</label>
                                    <div class="layui-input-block">
                                        <input type="text" name="comic[cid]" placeholder="Category ID filtering, multiple separated by commas, such as: 8,9,10" value="<?=$zyz['comic']['cid']?>" class="layui-input"/>
                                    </div>
                                </div>
                                <div class="layui-form-item w120">
                                    <label class="layui-form-label">API key:</label>
                                    <div class="layui-input-inline" style="display: block;width: auto;float: none;">
                                        <input lay-filter="mode2" type="radio" name="comic[pay]" value="0" title="No need"<?php if($zyz['comic']['pay'] == 0) echo ' checked';?>>
                                        <input lay-filter="mode2" type="radio" name="comic[pay]" value="1" title="On"<?php if($zyz['comic']['pay'] == 1) echo ' checked';?>>
                                    </div>
                                </div>
                                <div class="layui-form-item w120 bpay" style="<?php if($zyz['comic']['pay'] == 0) echo 'display:none;';?>">
                                    <label class="layui-form-label layui-form-required">authorization key:</label>
                                    <div class="layui-input-block">
                                        <textarea style="min-height:120px;" name="comic[token]" placeholder="Authorization key, one per line" class="layui-textarea"><?=implode("\n",$zyz['comic']['token'])?></textarea>
                                    </div>
                                </div>
