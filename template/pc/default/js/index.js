	...
	$.getJSON('//'+Mcpath.url+Mcpath.web+'index.php/api/rend/gift?callback=?', {mid:_mid}, function(res) {
        if(res.code == 1){
        	...
        	for (var i = 0; i < list.length; i++) {
        		str+='<div class="scroll-item"><img class="avatar" src="'+list[i].upic+'" alt="Surplus"><span>'+list[i].unichen+' gift <b>'+list[i].num+'</b> 1  <b class="hl">'+list[i].name+'</b> </span></div>'
        	}
...
var showTicket = function(_mid,_mname,_mticket,_mrank) {
    ...
    if(mccms.user.ticket == 0) {
    	ticketTpl += '<div class=\"ticket__pay\">\n<span class=\"ticket__null\">Insufficient monthly pass, click me to buy</span></div>\n';
    }else{
    	ticketTpl += '<div data-id=\"'+_mid+'\" class=\"ticket__confirm\">\n      <span class=\"j-loading-icon loading-icon hide\">\n<i class=\"layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop\"></i>\n      </span>\n        <span>Vote now</span>\n    </div>\n';
    }
    ticketTpl += '<p class=\"my-ticket\">my Power <b class=\"hl\">'+mccms.user.ticket+'</b> </p>\n</div>\n';
    layer.open({
        type: 1,
        closeBtn: 0,
        title: false,
    ...
    $('.j-comment-submit').click(function(){
    	if(mccms.user.log == 0){
	    	...
	    	if($('.comment-kuang').hasClass('has-placeholder')) {
	    		mccms.msg('Please enter the content of the comment');
	    	}else{
...
$(function(){
	...
	$('.in-header__avatar').hover(function() {
		if(mccms.user.log == 1){
			$('.j-user-avatar').attr('src',mccms.user.pic);
			if(mccms.user.vip == '0') mccms.user.viptime = 'has expired';
