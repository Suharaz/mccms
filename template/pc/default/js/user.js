...
var infopass = function(){
    ...
    // 自定义验证
    mccms.form.verify({
        password: [/^[\S]{6,16}$/, 'Password must be 6 to 16 characters, and no spaces'],
    ...
    function edit_info() {
        mccms.layer.open({
            ...
            yes: function(idx) {
                ...
                if(!(/^[\S]{6,16}$/.test(pass))){
                    layer.tips('Password must be 6 to 16 characters, and no spaces','#dialog-setting-userinfo input[name=pass]',{tips: 1});
                    return;
                }
                if (pass != pass2) {
...
var message = function(){
    ...
    $('.del-btn').click(function(){
        ...
        if(ids.length == 0){
            mccms.msg('Please select the data to delete');
        }else{
