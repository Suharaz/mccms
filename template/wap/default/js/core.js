...
var user = {
    ...
    regpass : function(type){
	    ...
	    $('.register-btn').click(function(){
	        ...
	        if(!(/^[\S]{6,16}$/.test(pass))){
	            mccms.msg('Password must be 6 to 16 characters, and no spaces~',1);
	            $('#regpass').focus();
	            return false;
	        }
	        var index = mccms.layer.open({type: 2});
    ...
    infopass : function(){
    	...
    	$('.info-btn').click(function(){
	        ...
	        if(!(/^[\S]{6,16}$/.test(pass))){
	            mccms.msg('原Password must be 6 to 16 characters, and no spaces~',1);
	            $('.user-pass').focus();
	            return false;
	        }
	...
	payvip : function(cionname){
		...
		$('.J_buy_vip').click(function(){
		    ...
		    if(pay == 'cion'){
		    	mccms.layer.open({
				    content: 'need'+cion+'个'+cionname+'are you sure to buy?',
				    btn: ['Yes', 'Cancel'],
				    yes: function(index){
				    	paysave(day,pay);
				    	mccms.layer.close(index);
	...
	read : function(){
		...
		$('.delete').click(function(){
			...
			if(ids.length == 0){
				mccms.msg('Please select the data to delete',1);
			} else {
		        mccms.layer.open({
